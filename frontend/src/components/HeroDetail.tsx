...
const FALLBACK_IMG = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="400" height="225" viewBox="0 0 400 225" fill="none"><rect width="400" height="225" rx="16" fill="%23222"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="%23aaa" font-family="sans-serif" font-size="20">No Image</text></svg>';
  ...
  return (
    <div className="w-[min(92vw,900px)] max-h-[85vh] overflow-y-auto rounded-2xl bg-white/6 border border-white/10 p-6 shadow-2xl backdrop-blur-md relative">
      ...
      <header className="mb-4 flex flex-col gap-2">
        ...
        <p className="text-white/70 text-sm">Real name: <span className="text-white/90">{hero.real_name || 'Unknown'}</span></p>
      </header>
        ...
        <section className="mb-6">
          <div className="aspect-video w-full overflow-hidden rounded-lg border border-white/10 bg-white/10 flex items-center justify-center mb-3">
            <img
              ...
              alt={hero.nickname}
              onError={(event) => { if (event.currentTarget.src !== FALLBACK_IMG) event.currentTarget.src = FALLBACK_IMG; }}
      ...
      <section className="space-y-4 text-sm leading-relaxed">
        {hero.origin_description && (
          <div>
            <h3 className="font-semibold text-white/90 mb-1 text-sm uppercase tracking-wide">Origin</h3>
            <p className="text-white/70 whitespace-pre-line">{hero.origin_description}</p>
          </div>
        )}
        {hero.superpowers && (
          <div>
            <h3 className="font-semibold text-white/90 mb-1 text-sm uppercase tracking-wide">Superpowers</h3>
            <p className="text-white/70 whitespace-pre-line">{hero.superpowers}</p>
          </div>
        )}
        {hero.catch_phrase && (
          <div>
            <h3 className="font-semibold text-white/90 mb-1 text-sm uppercase tracking-wide">Catch phrase</h3>
            <blockquote className="text-white italic border-l-4 border-blue-500 pl-3">{hero.catch_phrase}</blockquote>
          </div>
        )}
